...
class EffectDB
{
    ...
    public static function getIdByEffectName($effect) {
        $resultatenArray = array();
        $resultaat = self::getVerbinding()->voerSqlQueryUit("SELECT * FROM Effect WHERE EffectName LIKE '%$effect%'");
        for ($index = 0; $index < $resultaat->num_rows; $index++) {
            $databaseRij = $resultaat->fetch_array();
            $nieuw = self::converteerRijNaarEffect($databaseRij);
            $resultatenArray[$index] = $nieuw;
    ...
    public static function ifExists($effect){
        ...
        $result = self::getVerbinding()->voerSqlQueryUit("SELECT * FROM Effect WHERE EffectName LIKE '%$effect%'");
